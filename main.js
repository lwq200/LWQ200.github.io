...
(function () {
    ...
    function updateDeviceClass() {
        ...
        const mobile = isMobileDevice || w < 768;
        body.classList.remove('is-desktop', 'is-mobile', 'is-landscape', 'is-portrait', 'is-foldable');
        body.classList.add(mobile ? 'is-mobile' : 'is-desktop');
        body.classList.add(w >= h ? 'is-landscape' : 'is-portrait');
        // 折叠屏内屏典型展开尺寸：宽 600~900px 且接近方形
        const ratio = w / h;
        if (w >= 600 && w <= 900 && ratio > 0.75 && ratio < 1.5) {
            body.classList.add('is-foldable');
        }
        // 设置真实视口高度变量，兼容老旧浏览器（不支持 dvh 时回退）
        document.documentElement.style.setProperty('--app-vh', h * 0.01 + 'px');
        document.documentElement.style.setProperty('--app-vw', w * 0.01 + 'px');
    }
    updateDeviceClass();

    // 防抖：避免 resize 期间频繁触发布局重排
    let resizeTimer = null;
    function onViewportChange() {
        if (resizeTimer) cancelAnimationFrame(resizeTimer);
        resizeTimer = requestAnimationFrame(function () {
            updateDeviceClass();
            // 触发自定义事件，便于其他模块（如浮层重定位）响应
            try {
    ...
    // 监听 visualViewport（处理虚拟键盘弹出、地址栏隐藏/显示）
    if (window.visualViewport) {
        window.visualViewport.addEventListener('resize', onViewportChange, { passive: true });
    }
    // 监听屏幕折叠展开（Galaxy Fold、Surface Duo 等）
    if (window.matchMedia) {
        try {
            const foldQuery = window.matchMedia('(spanning: single-fold-vertical), (spanning: single-fold-horizontal)');
            if (foldQuery.addEventListener) foldQuery.addEventListener('change', onViewportChange);
            else if (foldQuery.addListener) foldQuery.addListener(onViewportChange);
        } catch (_) { /* 不支持时静默 */ }
    }

    /* ================================================================
       移动端自动重定向弹窗 (已禁用，统一使用桌面版)
       ================================================================ */
    // const MOBILE_REDIRECT_URL = '#';
    // const STORAGE_KEY = 'stay_desktop';
    // function shouldShowMobileRedirect() { ... }
    // if (shouldShowMobileRedirect()) { setTimeout(showMobileRedirectModal, 600); }
    ...
    function initNavigation() {
        ...
        if (dropdownToggle && dropdownMenu) {
            // 桌面端动态修正下拉菜单位置，确保不溢出屏幕（移动端样式已用 fixed 全宽兜底）
            function clampDropdownPosition() {
                ...
                if (rect.right > window.innerWidth - margin) {
                    ...
                    dropdownMenu.style.right = (margin) + 'px';
            ...
            dropdownToggle.addEventListener('click', function (e) {
                ...
                if (expanded) requestAnimationFrame(clampDropdownPosition);
            });
            ...
            // 视口变化时重新修正下拉位置 / 自动收起以避免错位
            window.addEventListener('app:viewport-change', function () {
